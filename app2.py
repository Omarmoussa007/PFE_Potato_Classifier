...
advice = {
    "Potato___Early_blight": {
        ...
        "cause": "Caused by le champignon Alternaria solani, souvent dû à l'humidité et chaleur modérée.",
    ...
    "Potato___Late_blight": {
        ...
        "cause": "Caused by Phytophthora infestans, favorisé par humidité élevée et pluie.",
...
# 🔹 Dossier où sont stockées les images reçues
UPLOAD_FOLDER = r"C:\Users\21628\Downloads\esp32CAM\image_test"
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

# 🔹 Configuration de la page
st.set_page_config(
    page_title="Classification des maladies de la pomme de terre",
    layout="wide"
)
...
st.sidebar.header("📤 Ajouter une nouvelle image")
...
# 🔹 Fonction prédiction avec barre de progression
def predict_image(img_path):
    ...
    progress_text = "Prédiction en cours..."
...
for i, img_path in enumerate(all_images):
    ...
    with cols[i % 3]:
        ...
        st.markdown(f"<span style='color:{color}; font-weight:bold'>{emoji} {class_name} ({confidence:.1f}%)</span>", unsafe_allow_html=True)
        st.markdown(f"**Description:** {advice[class_name]['description']}")
        st.markdown(f"**Cause possible:** {advice[class_name]['cause']}")
        st.markdown(f"**Conseils:** {advice[class_name]['help']}")
        st.markdown("---")

# 🔹 Prédiction pour nouvelle image uploadée
if uploaded_file is not None:
    st.subheader("🆕 Nouvelle image uploadée")
    ...
    # 🎉 Animation de succès pour nouvelle image
    st.balloons()